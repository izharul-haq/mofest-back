...
import authorizeUser from '~/app/auth/authorizeUser';
...
export default (): Resource =>
  <Resource>{
    put: {
      schema: {
        description: 'Delete vote for a movie based on ID from user based on ID',
      ...
      handler: async (
        ...
        try {
          ...
          const view = await handler.unvote(userId, movieId);
